    ...
    <div class="breadcrumbs">
        ...
        <div class="crumb-container-collapsed">
            ...
            <span v-for="(link, index) in crumbsVisible" :key="getNodeKey(link)" class="crumb">
                <router-link v-if="crumbsVisible.length -1 != index" :to="link.to" @click="nodeClicked(link)" class="crumb-link" style="color: white">{{link.text}}</router-link>
                <span v-else class="crumb-no-link" style="color: white">{{link.text}}</span> <!--este es el apartado superior en el que aparece al lado del regresar en el header-->
