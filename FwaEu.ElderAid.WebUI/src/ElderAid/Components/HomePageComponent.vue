<template>
	<div class="page-home">
		<div
			class="flex-section justify-content-center"
			v-if="isSingleOrganization"
		>
			<span class="organization-text" v-if="organizations.length > 0">{{
				organizations[0].name
			}}</span>
			<span class="organization-text" v-else
				>Vous n'êtes affecté à aucun EMS (base de données)</span
			>
		</div>
		<div v-else class="change-organization-container">
			<span @click="goToOrganizationSelectionPage">{{
				organization?.name
			}}</span>
		</div>
		<div v-if="this.patientsActive.length > 0" class="vignette-list">
			...
			<div class="vignette-item">
				<div @click="goToPatientPage" class="vignette-main-info">
					<i
						class="fa-regular fa-user fa-fw vignette-icon"
						style="color: #94a595"
					/>
			...
			<div @click="goToOrdersPage" class="vignette-item">
				<div class="vignette-main-info">
					<i
						class="fa-regular fa-cart-plus fa-fw vignette-icon"
						style="color: #bda6a0"
					/>
			...
			<div @click="goToPeriodicPage" class="vignette-item">
				<div class="vignette-main-info">
					<i
						class="fa-regular fa-repeat vignette-icon fa-fw"
						style="color: #d8b291"
					/>
			...
			<div class="vignette-item" style="opacity: 0.4">
				<div class="vignette-main-info">
					<i
						class="fa-regular fa-truck vignette-icon fa-fw"
						style="color: #a5ae9d"
					/>
					<span class="vignette-text">Entrega (en desarrollo)</span>
				</div>
				<i class="fa-regular fa-angle-right chevron-icon" />
			</div>
			...
			<div @click="goToProfilPage" class="vignette-item">
				<div class="vignette-main-info">
					<i
						class="fa-regular fa-gear vignette-icon fa-fw"
						style="color: #bb8a7c"
					/>
...
export default {
	...
	created: showLoadingPanel(async function () {
		await loadMessagesAsync(
			this,
			import.meta.glob("./Content/home-page-messages.*.json")
		);
		...
		this.protections = (await ProtectionMasterDataService.getAllAsync()).filter(
			(x) => new Date(x.dateEnd) > new Date()
		);
	...
	methods: {
		...
		getNumberOfPatientToValidate() {
			...
			const patientsToValidate = this.protections
				...
				.filter(
					(patientId, index, array) =>
						patientId &&
						!this.isPatientInPeriodicOrders(patientId) &&
						array.indexOf(patientId) === index
				).length;

			return `${patientsToValidate} ${
				patientsToValidate !== 1 ? "patients" : "patient"
			} à valider`;
		},
