...
{
	public class BeforeUpdateFilesImportSubTask : LocalFilesImportSubTask, IBeforeUpdateSchemaSubTask
	{
		public BeforeUpdateFilesImportSubTask(IImportService importService,
			IPathFileProvider filesProvider,
			IOptions<SetupOptions> settings)
			: base(importService)
		{
			this._paths = settings.Value.BeforeUpdateSchemaPaths;
